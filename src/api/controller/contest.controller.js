...
const create = async (user, contestData, socket, cards) => {
    try {
        console.log('contestdata in create - ',contestData, ' and user - ',user);
        if (user.coins < contestData.betamount) {
            throw new Error('Not enough amount')
        }
        const contest = await ContestModel.create({ ...contestData, status: 'open', users: [{ ...user, rank: 0 }] })
        const ack = await UserModel.updateOne({ userID: user.userID },
            {
                $push: {
        ...
        socket.emit('join', { success: true, response: { status: 'open', myTurn: true, contest, cards } })
...
const addUser = async (user, contestData, socket, cards, oc) => {
    try {
        ...
        const opp = await UserModel.findOne({ userID: contestData.users[0].userID })
        ...
        socket.emit('join', { success: true, response: { status: 'full', myTurn: false, contest, cards, opponent: opp, oppCards: oc } })
        socket.broadcast.to(contestID).emit('OPP_JOIN', { success: true, response: { status: 'full', cards: oc, opponent: user, oppCards: cards } })
    } catch (error) {
...
exports.getContestUsers = async (req, res, next) => {
    try {
    ...
    .on('connection', (socket) => {
        ...
        try {
            socket.on('END', async ({ winnerID, looserID, contestID, won }) => {
                try {
                    console.log('end event triggered - ', winnerID, looserID, contestID);
